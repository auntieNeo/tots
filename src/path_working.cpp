...
namespace Tots
{
  Path::Path(const std::string &name, double x, double y, double initialSpeed) : Movement(name)
  {
    m_initialPoint.m_x = x;
    m_initialPoint.m_y = y;
    m_curveInitialPoint.m_x = 0;
    m_curveInitialPoint.m_y = 0;
  ...
  DoublePoint Path::step(double seconds)
  {
    ...
    {
      p = m_curveInitialPoint;
      p.m_x += m_initialPoint.m_x;
      p.m_y += m_initialPoint.m_y;
      return p;
    }
    m_curveTime += seconds;
    ...
    // p is relative to the curve, but the return value is relative to the initial point
    p.m_x += m_initialPoint.m_x;
    p.m_y += m_initialPoint.m_y;
